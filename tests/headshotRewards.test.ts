...
function hitZombie(weaponId: WeaponId, part: ZombieHitPart, lethal: boolean) {
  ...
  (mode as unknown as { ctx: unknown }).ctx = {
    ...
    hud: { showHitmarker: () => undefined },
    audio: { playZombieHit: () => undefined, playZombieDeath: () => undefined },
  ...
  return { calls, points: economy.points };
}
...
describe('global firearm headshot rewards', () => {
  ...
  it('keeps body-hit scoring unchanged', () => {
    expect(hitZombie('m4a1', 'torso', false).points).toBe(10);
  });

  it('awards a lethal headshot exactly once', () => {
    expect(hitZombie('m4a1', 'head', true).points).toBe(HEADSHOT_POINTS);
  });
});
